{
    ...
    // Declaring a variable with 'any' type (can hold any value)
        ...
        (anything as string);  // Now TypeScript will suggest string methods

        // Type assertion: telling TypeScript to treat 'anything' as a number
        (anything as number);  // Now TypeScript will suggest number methods
        ...
        const result1 = kgToGm(22) as number;

        // Calling the function with a string and asserting the result as string
        const result2 = kgToGm("22") as string;


        // Custom error type with a message property
        type CustomError = {
            message: string;
        }
